...
export default function TripsPage() {
  ...
  const { data: trips = [], isLoading } = useQuery({
    ...
    queryFn: () => tripsApi.list({ date: selectedDate }) as any,
  ...
  const createTrip = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['trips', selectedDate] });
      setShowCreate(false);
      setForm(emptyForm);
    },
    onError: (err: any) => {
  ...
  // Mise à jour temps réel des sièges — rafraîchit la liste
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm text-gray-500">
          {(trips as any[]).length} voyage(s) trouvé(s)
        </span>
      </div>
      ...
      <FormModal
        ...
        description="Planifiez un voyage en sélectionnant l'itinéraire et les ressources"
        size="lg"
        footer={
          <>
            <button
