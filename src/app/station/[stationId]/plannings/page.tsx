...
export default function StationPlanningsPage() {
  ...
  const { data: schedules = [], isLoading } = useQuery({
    queryKey: ['schedules'],
    queryFn: async () => ((await schedulesApi.list()) ?? []) as any[],
  });
  ...
  const generateAllMutation = useMutation({
    ...
    onSuccess: (res: any) => {
      ...
      qc.invalidateQueries({ queryKey: ['station-trips', stationId] });
    },
    onError: (e: any) => toast.error(e?.response?.data?.message ?? 'Erreur'),
  });
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Plannings de départ</h1>
          <p className="text-sm text-gray-500 mt-0.5">
            Configurez des départs récurrents par ligne
          </p>
        </div>
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1 flex-wrap">
                        ...
                        <span className="text-xs font-medium text-brand-600">{s.departureTime}</span>
                        <span className="text-xs text-gray-500">
            ...
            <div className="px-6 py-5 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Véhicule par défaut
                    <span className="text-gray-400 font-normal ml-1">(requis pour génération)</span>
                  </label>
                  <select
